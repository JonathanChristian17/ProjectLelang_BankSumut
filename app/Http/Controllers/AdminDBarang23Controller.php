<?php namespace App\Http\Controllers;
	...
	class AdminDBarang23Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Keterangan Bulanan","name"=>"ket_bulanan","join"=>"k_bulanan,status","callback"=>function($row) {  if(empty($row->ket_bulanan)) { return '<a href="#" id="ket_bulanan" class="editable" data-type="select" data-pk="'.$row->id.'" data-url="'.CRUDBooster::mainpath('save-ket').'" data-title="Pilih Keterangan bulanan" class="editable editable-click" style="color: gray;" data-original-title="" title="">Belum Dipilih</a>'; } else { return $row->ket_bulanan; } }];
			...
			$this->form[] = ['label'=>'Nilai Limit','name'=>'harga','type'=>'money','validation'=>'required|min:0|integer','width'=>'col-sm-10','decimals'=>'2','dec_point'=>','];
	        ...
	        */
			...
			$(document).ready(function() { 
				$('.editable').editable({ source: function() {  
					return '".CRUDBooster::mainpath('ket')."' } 
				}); 
			}); "; 
	        ...
	        */
	        $this->load_js = array();
			$this->load_js[] = '//cdnjs.cloudflare.com/ajax/libs/x-editable/1.5.0/bootstrap3-editable/js/bootstrap-editable.min.js'; 
	        ...
	        */
	        $this->load_css = array();
			$this->load_css[] = '//cdnjs.cloudflare.com/ajax/libs/x-editable/1.5.0/bootstrap3-editable/css/bootstrap-editable.css'; 
	        
	        
	    }
		public function getKet() { 
			if(Request::ajax()){ 
				$ketbul = DB::table('k_bulanan')->get(); 
				foreach ($ketbul as $value) { 
					$list_ketbul[] = [ 'value'=> $value->id, 'text'	=> $value->status ];
				} 
				return response()->json($list_ketbul); 
			} 
		} 

		public function postSaveKet() {  
			if(Request::ajax()){ 
				DB::table('d_barang')->where('id',Request::get('pk'))->update(['ket_bulanan'=>Request::get('value')]); 
